...
(function() {
  var log = function(txt) {
    if (console && console.log) console.log("Hornet: " + txt);
  };
  ...
  delayedTimeout.prototype = {
    nextTick: function() {
      if (!this.previousTime) this.previousTime = 0;
      ...
      if (newTime > this.maxTime) newTime = this.maxTime;
  ...
  Hornet = function(uri, channels, token) {
    if (arguments.length == 1 && typeof arguments[0] == "object") {
      ...
      // Deprecated
      log(
        "Using constructor with multiples params instead of json object is deprecated and will be removed in Hornet 0.4"
      );
  ...
  Hornet.prototype = {
    ...
    addHandler: function(channel, type, callback) {
      ...
      if (!(channel in handlers)) handlers[channel] = {};

      if (!(type in handlers[channel])) handlers[channel][type] = [];
    ...
    removeHandler: function(channel, type, callback) {
      ...
      if (!(channel in handlers) || !(type in handlers[channel])) return; // do nothing...
    ...
    connect: function() {
      ...
      if (firstTime) {
        socket.on("connect", function() {
          that.connected = true;
          socket.send(
            JSON.stringify({ token: that.token, channels: that.channels })
          );
        });
        ...
        socket.on("message", function(rawMessage) {
          ...
          if (!(channel in handlers) || !(type in handlers[channel])) return;
        ...
        socket.on("disconnect", function() {
          ...
          if (
            handlers["hornet"] != undefined &&
            "disconnect" in handlers["hornet"]
          ) {
          ...
          var reconnect = new delayedTimeout(
            function(self) {
              if (!that.connected) that.socket.socket.connect();
              else self.stop();
            },
            1000,
            30000
          ); // will try to reconnect after 1sec and will up to 30sec
        });
      }
    }
  };
