...
describe("Client", function() {
  describe("connected on one single channel", function() {
    it("should receive a message when one is published on redis", function(done) {
      ...
      setTimeout(function() {
        redis.publish(
          consts["HORNET_USER_SPECIFIC_CHANNEL_PREFIX"] + channel,
  ...
  describe("connected on multiples channel", function() {
    it("should receive a message when one is published on redis", function(done) {
      ...
      setTimeout(function() {
        redis.publish(
          consts["HORNET_USER_SPECIFIC_CHANNEL_PREFIX"] + channel1,
          JSON.stringify({ type: "test" })
        );
        redis.publish(
          consts["HORNET_USER_SPECIFIC_CHANNEL_PREFIX"] + channel2,
  ...
  describe("connected on one single channel", function() {
    ...
    it("should receive a message for one client if except is in message", function(done) {
      ...
      setTimeout(function() {
        redis.publish(
          consts["HORNET_USER_SPECIFIC_CHANNEL_PREFIX"] + channel,
          JSON.stringify({ type: "test", except: "otken" })
        );
      }, 100);
    ...
    it("should receive a message for one client if only is in message", function(done) {
      ...
      setTimeout(function() {
        redis.publish(
          consts["HORNET_USER_SPECIFIC_CHANNEL_PREFIX"] + channel,
          JSON.stringify({ type: "test", only: "otken" })
        );
      }, 100);
