...
HornetChannel = function(settings, name) {
  ...
  this.name = name
  this.clients = []
  this.redisPub = redis.createClient(consts['REDIS_URL'])
}
...
HornetChannel.prototype = {
  ...
  addClient: function(client) {
    ...
    this.logger.log(
      'Channel:' + this.name + ' - connected token:' + client.token,
      module
    )
    ...
    client.on('disconnect', function() {
      ...
      that.logger.log(
        'Channel:' + that.name + ' - disconnected token:' + token,
        module
      )
  ...
  broadcast: function(message) {
    console.log(
      'Channel: ' + this.name + ' - new message:' + JSON.stringify(message)
    )

    this.logger.log(
      'Channel: ' + this.name + ' - new message:' + message,
      module
    )
    ...
    for (var token in clients) {
      ...
      // then continue
      if ('except' in message && message['except'] == client.token) continue
