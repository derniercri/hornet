...
function ChannelsHub( settings ) {
  ...
  this.redis.on("subscribe", function ( channel, count ) {
    that.logger.log("Subscribed to channel:" + channel + " - connected subscribers:" + count, module);
  });
...
ChannelsHub.prototype = {
  ...
  subscribe : function ( channelName , client ) {
    ...
    if ( ! ( channelName in this.channels ) ) {  
      ...
      this.logger.log('Channel:' + channelName + ' - subscribing token:' + client.token , module);

      this.redis.subscribe( consts['HORNET_USER_SPECIFIC_CHANNEL_PREFIX'] + channelName );
    }
