...
Router.prototype = {
  ...
  set: function(route, routeName, callback) {
    ...
    while (paramX != -1) {
      ...
      paramName =
        paramY != -1
          ? route.substr(paramX + 1, paramY - paramX - 1)
          : route.substr(paramX + 1);
      paramsNames.push(paramName);

      route =
        route.substr(0, paramX) +
        "(\\w+)" +
        (paramY != -1 ? route.substr(paramY) : "");
    ...
    this.logger.log(
      "Added route - name:" + routeName + " - format:" + routeInit,
      module
    );
  },
  ...
  serve: function(request, response) {
    ...
    this.logger.log("no route found for: " + request.url, module);
    hornetRequest.die();
  }
};
