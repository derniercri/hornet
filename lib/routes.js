...
var root = function() {
  ...
  hornetRequest.response.end(
    "<html><head><title>Hornet</title></head><body><h1>Hornet v" +
...
var hornetJs = function() {
  ...
  fs.readFile(__dirname + "/../public/hornet.js", "binary", function(
    err,
    file
  ) {
    if (err) {
    ...
    fs.readFile(
      ...
      "binary",
      function(err, socketioMin) {
        if (err) {
          hornetRequest.response.writeHeader(500, {
