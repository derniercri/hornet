...
function Hornet(options) {
  ...
  // redis client
  this.redis = redis.createClient(consts['REDIS_URL'])
...
Hornet.prototype = {
  ...
  disconnectInvalidToken: function(socket, token) {
    ...
    this.logger.log(
      "A client tried to connect with an unregistred token:'" + token + "'",
      module
    )
  },
  ...
  disconnectWrongMessageFormat: function(socket) {
    this.logger.log(
      'Connection error: wrong parameters: channel and/or token were missing',
      module
    )
    ...
    var msg = {
      ...
      errorMsg:
        'Client connection error: Wrong parameters. You should connect to hornet with channels and token parameters'
    }
      ...
      that.redis.smembers('hornet:token:' + token, function(err, reply) {
        ...
        if (found) {
          ...
          for (var i in channels) {
            ...
            that.logger.log(
              'Access granted for token:' +
                token +
                ' on channel:' +
                channel +
                '. Starting connected mode.',
              module
            )
  ...
  listen: function() {
    var that = this
    this.logger.log(
      'Starting Hornet Server v' +
        consts['HORNET_VERSION'] +
        ' on port: ' +
        this.settings['port'],
      module
    )
    ...
    this.io.sockets.on('connection', function(client) {
      that.logger.log(
        'New client connection - ' +
          JSON.stringify(client['handshake']['address']) +
          ' - ' +
          JSON.stringify(client['handshake']['headers']),
        module
      )
    ...
    this.logger.log(
      'Hornet Server started, listening any connections...',
      module
    )
